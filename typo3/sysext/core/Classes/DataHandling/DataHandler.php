...
class DataHandler {
	...
	public function clear_cache($table, $uid) {
		...
		array_push(static::$recordsToClearCacheFor[$table], $uid);
	}

	/**
	 * Do the actual clear cache
	 */
	public function processClearCacheQueue() {
		$cachesToClear = $this->getCachesToClearWhenPageCacheIsCleared();
		$list_cache = array();
		$tagsToClear = array();
		...
		foreach (static::$recordsToClearCacheFor as $table => $uids) {
			foreach (array_unique($uids) as $uid) {
				...
				if (is_array($GLOBALS['TCA'][$table]) && $uid > 0) {
					...
					if (!$TSConfig['clearCache_disable']) {
						...
						if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('cms')) {
							if ($table === 'pages' || $table === 'pages_language_overlay') {
								...
								while ($row_tmp = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_tmp)) {
									...
									if ($TSConfig['clearCache_pageSiblingChildren']) {
										...
										while ($row_tmp2 = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_tmp2)) {
											array_push($tagsToClear,'pageId_' . $row_tmp2['uid']);
										}
								...
								array_push($tagsToClear,'pageId_' . $pid_tmp);
								...
								if ($TSConfig['clearCache_pageGrandParent']) {
									...
									if ($row_tmp = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_tmp)) {
										array_push($tagsToClear,'pageId_' . $row_tmp['pid']);
									}
									$GLOBALS['TYPO3_DB']->sql_free_result($res_tmp);
								}
							} else {
								...
								array_push($tagsToClear,'pageId_' . intval($this->getPID($table, $uid)));
							}
							...
							array_push($tagsToClear, $table . '_' . $uid);
						}
					}
					...
					if ($TSConfig['clearCacheCmd']) {
						...
						$clearCacheCommands = array_merge($clearCacheCommands, $Commands);
					}
			...
			// Delete cache for current table and record
			array_push($tagsToClear, $table);
		}

		// Delete cache for selected tags
		foreach (array_unique($tagsToClear) as $cacheTag) {
			if (strpos($cacheTag,'pageId_') > 0) {
				foreach ($cachesToClear as $cache) {
					$cache->flushByTag($cacheTag);
				}
			} else {
				$GLOBALS['typo3CacheManager']->flushCachesByTag($cacheTag);
			}
		}

		// Execute collected clear cache commands from page TSConfig
		foreach (array_unique($clearCacheCommands) as $cmdPart) {
	...
	public function clear_cacheCmd($cacheCmd) {
		...
		if (count($tagsToFlush) > 0) {
			foreach ($this->getCachesToClearWhenPageCacheIsCleared() as $cache) {
	...
	/**
	 * Return an array of caches that should be cleared when pages are cleared
	 *
	 * @return array<\TYPO3\CMS\Core\Cache\Frontend\FrontendInterface>
	 */
	public function getCachesToClearWhenPageCacheIsCleared() {
		$listOfCachesToClearFor = $GLOBALS['TYPO3_CONF_VARS']['SYS']['cachesToClearWhenPagesAreChanged'];
		$cachesToClear = array();
		foreach (\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $listOfCachesToClearFor) as $cacheName) {
			array_push($cachesToClear, $GLOBALS['typo3CacheManager']->getCache($cacheName));
		}
